...
export class PillLogsController {
    ...
    // Dohvata sve logove jednog korisnika
    ...
    // Dohvata logove koji su uzeti danas za korisnika
    static async getTodayLogsByUser(req: Request, res: Response) {
        ...
        try {
            ...
            const [rows]: any = await pool.query(
                `SELECT p.id as pill_id, p.name, p.dosage, p.time, p.note, p.image, p.count,
                        pl.id as log_id, pl.status, pl.taken_at
                 FROM pills p
    ...
    // Kreira novi log unosa leka
    static async createLog(req: Request, res: Response) {
        try {
            const { pill_id, status, taken_at } = req.body;
            if (!pill_id || !status) return res.status(400).json({ message: 'Nedostaju obavezni podaci' });

            const [result] = await pool.query<ResultSetHeader>(
                `INSERT INTO pill_logs (pill_id, status, taken_at) VALUES (?, ?, ?)`,
    ...
    static async updateLog(req: Request, res: Response) {
        try {
            ...
            if (result.affectedRows === 0) return res.status(404).json({ message: 'Log nije pronađen za izmenu' });
    ...
    static async deleteLog(req: Request, res: Response) {
        try {
            ...
            if (result.affectedRows === 0) return res.status(404).json({ message: 'Log nije pronađen za brisanje' });
